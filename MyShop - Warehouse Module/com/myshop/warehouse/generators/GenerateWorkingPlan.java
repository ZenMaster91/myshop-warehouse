...
public class GenerateWorkingPlan {
	...
	private static WorkingPlanController generateFromList(List<Order> orders) {
		WorkingPlanController workingPlan = new WorkingPlanController();
	...
	private static List<WorkingPlanController> generateFromSingle(Order o) {
		...
		workingPlan = new WorkingPlanController();
		WorkingPlanController parent = workingPlan;
		List<OrderItem> toDelete = new ArrayList<OrderItem>();
		for (OrderItem oi : o.getProducts()) {
			if (workingPlan.getNumberOfItems() < 1 && (oi.getProductWeight() > WorkingPlanGenerator.MAX_WP_LOAD && oi.getProduct().getDimensions().calculateVolume() > WorkingPlanGenerator.MAX_WP_VOL)) {
				workingPlan.addItem(oi);
				toDelete.add(oi);
				if (!workingPlan.isChild() && workingPlan.getNumberOfItems() > 0)
					workingPlans.add(workingPlan);
				workingPlan = new WorkingPlanController().setParent(parent);
			}
		}
		...
		while (i < o.getProducts().size()) {
			...
			while (j < o.getProducts().size()) {
				if ((workingPlan.getTotalWeight() + o.getProducts().get(j).getProductWeight() <= WorkingPlanGenerator.MAX_WP_LOAD) && (workingPlan.getTotalVolume() + o.getProducts().get(j).getProduct().getDimensions().calculateVolume() <= WorkingPlanGenerator.MAX_WP_VOL)) {
	...
	private static List<WorkingPlanController> generate(Order order, List<Order> orders) {
		if (orders.contains(order)) {
			...
			if (new FactorEstiba(new OrderController(order).getVolume(), new OrderController().getWeight(order))
				...
				return generateFromSingle(order);
			}
		}
	}
	
	/**
	 * Generates and assigns the generated working plans to the given warehouse keeper.
	 * 
	 * @param wk the warehouse keeper
	 * @param order order to start the generation of orders.
	 * @param orders to generate from.
	 */
