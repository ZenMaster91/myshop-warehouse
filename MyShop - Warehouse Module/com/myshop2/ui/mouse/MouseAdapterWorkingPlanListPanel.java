...
public static MouseAdapter getFromTable(JTable table, WarehouseAPP app) { 
		...
		return new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				WorkingPlanController wp = ((DefaultNonEditableTableModel<WorkingPlanController>) table.getModel()).getObjectAtRow(table.getSelectedRow());
				((JScrollPane) app.getScPaneOTIndiv()).setViewportView(LoadDetallesWorkingPlan.loadFromTable(wp, app.getScPaneOTIndiv().getWidth()));
				app.getIdPedidoOTIndv().setText(Integer.toString(wp.getWp().getID()));
				SimpleDateFormat sf = new SimpleDateFormat("MMM. dd, yyyy hh:mm a");
				app.getFechaOTIndv().setText(sf.format(new Date()));
				app.getArticulosOTIdnv().setText(Integer.toString(wp.getNumberOfItems()));
				app.getPesoOTIndv().setText(Double.toString(wp.getTotalWeight()) + " Kg");
				((CardLayout) app.getContentPane().getLayout()).show(app.getContentPane(), "vistaOTIndividual");
			}
		};
	}
